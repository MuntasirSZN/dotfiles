-- Auto-generated by DMS Matugen hook — do not edit manually.
-- Remove require("dms.colors") from hyprland.lua to override.

hl.config({
	general = {
		col = {
			active_border = "rgb(82d3e1)",
			inactive_border = "rgb(899294)",
		},
	},
	group = {
		col = {
			border_active = "rgb(82d3e1)",
			border_inactive = "rgb(899294)",
			border_locked_active = "rgb(ffb4ab)",
			border_locked_inactive = "rgb(899294)",
		},
		groupbar = {
			col = {
				active = "rgb(82d3e1)",
				inactive = "rgb(899294)",
				locked_active = "rgb(ffb4ab)",
				locked_inactive = "rgb(899294)",
			},
		},
	},
})
